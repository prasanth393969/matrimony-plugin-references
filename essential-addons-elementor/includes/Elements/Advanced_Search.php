...
class Advanced_Search extends Widget_Base {
	...
	public function search_setting() {
		...
		$this->add_control(
			'Seacrh_among_heading',
			[
				'label' => esc_html__( 'Search Among', 'essential-addons-elementor' ),
		...
		$this->add_control(
			...
			[
				'label'        => esc_html__( 'Taxonomies', 'essential-addons-elementor' ),
		...
		if( class_exists( 'woocommerce' ) ) {
			$this->add_control(
				'search_among_sku',
				[
					'label'        => esc_html__( 'SKU', 'essential-addons-elementor' ),
					'type'         => Controls_Manager::SWITCHER,
					'return_value' => 'yes',
					'default'      => 'no',
				]
		...
		$this->add_control(
			...
			[
				'label'        => esc_html__( 'Include Current Page', 'essential-addons-elementor' ),
				'type'         => Controls_Manager::SWITCHER,
				'label_on'     => __( 'Yes', 'essential-addons-elementor' ),
				'label_off'    => __( 'No', 'essential-addons-elementor' ),
				'return_value' => 'yes',
				'default'      => 'no',
				'description'  => esc_html__( 'Include the current page in search results when relevant to the search query.', 'essential-addons-elementor' )
			]
		);
		...
		$this->add_control(
			'show_initial_result',
			[
				'label'   => esc_html__( 'Initial Results Count', 'essential-addons-elementor' ),
		...
		$this->add_control(
			'show_content_image',
			[
				'label'        => esc_html__( 'Content Image', 'essential-addons-elementor' ),
				'type'         => Controls_Manager::SWITCHER,
				'label_on'     => __( 'Show', 'essential-addons-elementor' ),
				'label_off'    => __( 'Hide', 'essential-addons-elementor' ),
				'return_value' => 'yes',
		...
		$this->add_control(
			'result_on_new_tab',
			[
				'label'        => esc_html__( 'Result Link on New Tab', 'essential-addons-elementor' ),
				'type'         => Controls_Manager::SWITCHER,
				'return_value' => 'yes',
				'default'      => 'no',
		...
		if ( is_multisite() && is_super_admin() && is_main_site() ) {
			$this->add_control(
				'multisite_search_heading',
				[
					'label' => esc_html__( 'Multisite Search', 'essential-addons-elementor' ),
					'type'  => Controls_Manager::HEADING,
					'separator' => 'before',
				]
			);
			...
			$this->add_control(
				...
				[
					'label'        => esc_html__( 'Enable Multisite Search', 'essential-addons-elementor' ),
					'type'         => Controls_Manager::SWITCHER,
					'label_on'     => __( 'Yes', 'essential-addons-elementor' ),
					'label_off'    => __( 'No', 'essential-addons-elementor' ),
					'return_value' => 'yes',
					'default'      => 'no',
					'description'  => esc_html__( 'Search across multiple sites in the network.', 'essential-addons-elementor' )
				]
			);
			...
			$this->add_control(
				...
				[
					'label'       => esc_html__( 'Select Sites', 'essential-addons-elementor' ),
					'type'        => Controls_Manager::SELECT2,
					'multiple'    => true,
					'label_block' => true,
					'options'     => $this->get_multisite_list(),
					'default'     => [],
					'condition'   => [
						'enable_multisite_search' => 'yes',
					],
					'description' => esc_html__( 'Leave empty to search all sites in the network.', 'essential-addons-elementor' )
				]
			);

			$this->add_control(
				'multisite_show_site_name',
				[
					'label'        => esc_html__( 'Show Site Name in Results', 'essential-addons-elementor' ),
					'type'         => Controls_Manager::SWITCHER,
					'label_on'     => __( 'Yes', 'essential-addons-elementor' ),
					'label_off'    => __( 'No', 'essential-addons-elementor' ),
					'return_value' => 'yes',
					...
					'description'  => esc_html__( 'Display the site name with each search result.', 'essential-addons-elementor' )
				]
			);
		}
	...
	public function get_multisite_list() {
		...
		$list = get_transient( 'eael_adv_search_multisite_list' );
		if ( $list && is_array( $list ) ) {
			return $list;
		}
		...
		$sites = get_sites( array(
			...
			'spam'     => 0,
			'deleted'  => 0,
			'number'   => 50, // Limit to 100 sites for performance
		) );
		...
		if ( ! empty( $sites ) ) {
			foreach ( $sites as $site ) {
				...
				if ( $site_details ) {
					$list[ $site->blog_id ] = $site_details->blogname . ' (' . $site_details->domain . $site_details->path . ')';
				}
			}
			set_transient( 'eael_adv_search_multisite_list', $list, 30 * MINUTE_IN_SECONDS );
		}
	...
	protected function render() {
		...
		$args = [
			...
			'include_current_page'           => isset( $settings[ 'include_current_page' ] ) ? $settings[ 'include_current_page' ] : 'no',
		];
		...
		if ( is_multisite() && is_main_site() ) {
			$enable_multisite_search = isset( $settings[ 'enable_multisite_search' ] ) ? $settings[ 'enable_multisite_search' ] : 'no';

			if ( 'yes' === $enable_multisite_search ) {
				$args[ 'enable_multisite_search' ] = $enable_multisite_search;
				$args[ 'multisite_search_sites' ] = isset( $settings[ 'multisite_search_sites' ] ) ? $settings[ 'multisite_search_sites' ] : [];
				$args[ 'multisite_show_site_name' ] = isset( $settings[ 'multisite_show_site_name' ] ) ? $settings[ 'multisite_show_site_name' ] : 'yes';
			}
		}
