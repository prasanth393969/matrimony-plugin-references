...
class Lightbox extends Widget_Base
{
		...
		$this->add_control(
			...
			[
				...
				'options' => [
					...
					'text' => [
						'title' => esc_html__( 'Text Link', 'essential-addons-elementor' ),
						'icon' => 'eicon-text',
					],
					'icon' => [
	...
	public function style_text_link(){
		$this->start_controls_section(
			'eael_section_lightbox_text_link_styles',
			[
				'label'     => esc_html__('Text Link', 'essential-addons-elementor'),
				'tab'       => Controls_Manager::TAB_STYLE,
				'condition' => [
					'eael_lightbox_trigger_type' => 'eael_lightbox_trigger_button',
					'trigger_type'               => 'text',
		...
		$this->add_group_control(
			Group_Control_Typography::get_type(),
			[
				'name'      => 'eael_lightbox_text_link_typography',
				'global' => [
					'default' => Global_Typography::TYPOGRAPHY_PRIMARY
				],
				'selector'  => '{{WRAPPER}} .eael-trigger-text'
			]
		);

		$this->start_controls_tabs('eael_lightbox_text_link_color_tabs');

		$this->start_controls_tab(
			'text_link_normal_tab',
			[
				'label' => esc_html__('Normal', 'essential-addons-elementor'),
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'default' => '#007cba',
				'selectors' => [
					'{{WRAPPER}} .eael-trigger-text' => 'color: {{VALUE}};',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'default' => '#005a87',
				'selectors' => [
					'{{WRAPPER}} .eael-trigger-text:hover' => 'color: {{VALUE}};',
				],
			]
		);
		...
		<div <?php $this->print_render_attribute_string('eael-lightbox-wrapper'); ?>>
			<div class="eael-lightbox-btn">
				...
				if (($settings['eael_lightbox_trigger_type']) == 'eael_lightbox_trigger_button') {
					...
					} else if ('text' == $settings['trigger_type']) {
						echo '<span class="eael-trigger-text eael-modal-popup-link eael-modal-popup-link-' . esc_attr($uniq_id) . '">';
						echo wp_kses( $settings['eael_lightbox_open_btn'], Helper::eael_allowed_tags() );
						echo '</span>';
