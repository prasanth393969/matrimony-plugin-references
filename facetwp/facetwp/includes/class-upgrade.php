    ...
    private function run_upgrade() {
        ...
        if ( version_compare( $this->last_version, '4.4', '<' ) ) {
            if ( ! isset( $settings['settings']['places_version'] ) ) {
                ...
                $api_key = $api_key ?: $settings['settings']['gmaps_api_key'];

                if ( apply_filters( 'facetwp_gmaps_api_key', $api_key ) ) {
                    $settings['settings']['places_version'] = 'places-service';
                } else {
                    $settings['settings']['places_version'] = 'place-class';
                }
                $changed = true;
            }
        }
